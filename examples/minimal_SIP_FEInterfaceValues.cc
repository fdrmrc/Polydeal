...
Poisson<dim>::assemble_system()
{
  ...
  FullMatrix<double> cell_matrix_faces{};
              ...
              if (cell->active_cell_index() < neigh_cell->active_cell_index())
                {
                  ...
                  auto &fe_faces0 = fe_faces.first;
                  auto &fe_faces1 = fe_faces.second;

                  FEInterfaceValues<dim> feiv(fe_faces0, fe_faces1);
                  feiv.reinit(cell, neigh_cell);
                  std::vector<types::global_dof_index>
                    local_dof_indices_interface(
                      feiv.n_current_interface_dofs());


                  cell_matrix_faces.reinit(feiv.n_current_interface_dofs(),
                                           feiv.n_current_interface_dofs());
                  const auto &normals = feiv.get_normal_vectors();
                  for (unsigned int q_index : feiv.quadrature_point_indices())
                    {
                      for (unsigned int i : feiv.dof_indices())
                        {
                          for (unsigned int j : feiv.dof_indices())
                            {
                              cell_matrix_faces(i, j) +=
                                (- //
                  ...
                  constraints.distribute_local_to_global(
                    cell_matrix_faces,
                    feiv.get_interface_dof_indices(),
                    system_matrix);
