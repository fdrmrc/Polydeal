                           ...
                           const unsigned int /*component*/) const
{
  ...
  if ((point[0] >= 0.4 && point[0] <= 0.6 && point[1] >= 0.4 &&
       point[1] <= 0.6) &&
      (t >= 0. && t <= t_end_current))
    return 2 * 1e6;
  else
...
private:
  ...
  double
  Iion(const double u_old, const double w) const;
  ...
  dealii::RTree<
    ...
    boost::geometry::index::rstar<4, 2, 2, 16>,
...
IonicModel<dim>::setup_problem()
{
  ...
  tree = pack_rtree<bgi::rstar<4, 2, 2, 16>>(boxes);
...
template <int dim>
double
IonicModel<dim>::Iion(const double u_old, const double w) const
{
  return param.kappa * u_old * (u_old - param.a) * (u_old - 1.0) + w;
}
...
IonicModel<dim>::update_w_and_ion()
{
  ...
  // After that, we evaluate Iion at DoFs
  for (const types::global_dof_index i : locally_owned_dofs)
    {
      locally_relevant_w0_current[i] =
        (locally_relevant_w0_pre[i] +
         dt * param.epsilon * locally_relevant_solution_pre[i]) /
        (1. + dt * param.gamma * param.epsilon);

      // Evaluate ion at u_n, w_{n+1}
      ion_at_dofs[i] =
        Iion(locally_relevant_solution_pre[i], locally_relevant_w0_current[i]);
    }
}
...
IonicModel<dim>::output_results()
{
  ...
  const std::string filename =
    ("2Dmonodomain_time_" + std::to_string(time) +
  ...
  if (Utilities::MPI::this_mpi_process(communicator) == 0)
    {
      ...
      for (unsigned int i = 0;
           ...
           i++)
        {
          filenames.push_back("2Dmonodomain_time_" + std::to_string(time) +
                              Utilities::int_to_string(i, 4) + ".vtu");
        }
      std::ofstream master_output("2Dmonodomain_time_" + std::to_string(time) +
                                  ".pvtu");
...
IonicModel<dim>::run()
{
  ...
  GridGenerator::hyper_cube(tria_dummy, 0., 1.);
  tria_dummy.refine_global(7);
  ...
  while (time <= end_time)
    {
      ...
      mass_matrix.vmult_add(
        system_rhs,
        locally_relevant_solution_pre); // Add to system_rhs (chi*Cm)/dt*M*un
