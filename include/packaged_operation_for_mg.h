...
#ifndef dealii_packaged_operation_for_mg_h
#define dealii_packaged_operation_for_mg_h
 ...
 * @endcode
 *
 * Similar to the LinearOperatorMG class it also has knowledge about how to
 ...
 * <code>PackagedOperationMG<Vector<double>></code>. It stores
 ...
 * @endcode
 * or until the @p PackagedOperationMG object is implicitly converted:
 * @code
 *   Vector<double> y;
 *   y = residual;
 *   y += residual;
...
public:
  /**
   * Create an empty PackagedOperationMG object. All <code>std::function</code>
  ...
  PackagedOperationMG()
  {
    apply = [](Range &) {
      Assert(false,
             ExcMessage(
               "Uninitialized PackagedOperationMG<Range>::apply called"));
    };

    apply_add = [](Range &) {
      Assert(false,
             ExcMessage(
               "Uninitialized PackagedOperationMG<Range>::apply_add called"));
    };

    reinit_vector = [](Range &, bool) {
      Assert(false,
             ExcMessage(
               "Uninitialized PackagedOperationMG<Range>::reinit_vector "
               "method called"));
    };
  }
  ...
  /**
   * Constructor that creates a PackagedOperationMG object from a reference
   * vector @p u. The PackagedOperationMG returns @p u.
   *
   * The PackagedOperationMG object that is created stores a reference to @p u.
   * Thus, the vector must remain a valid reference for the whole lifetime of
   * the PackagedOperationMG object. All changes made on @p u after the creation
   * of the PackagedOperationMG object are reflected by the operator object.
   */
   ...
   */
  PackagedOperationMG<Range> &
  operator=(const PackagedOperationMG<Range> &) = default;

  /**
   * Copy assignment operator that creates a PackagedOperationMG object from a
   * reference vector @p u. The PackagedOperationMG returns @p u.
   *
   * The PackagedOperationMG object that is created stores a reference to @p u.
   * Thus, the vector must remain a valid reference for the whole lifetime of
   * the PackagedOperationMG object. All changes made on @p u after the creation
  ...
  /**
   * Addition with a PackagedOperationMG @p second_comp with the same @p Range.
   */
  PackagedOperationMG<Range> &
  operator+=(const PackagedOperationMG<Range> &second_comp)
  {
  ...
  /**
   * Subtraction with a PackagedOperationMG @p second_comp with the same @p
   * Range.
   */
  PackagedOperationMG<Range> &
  operator-=(const PackagedOperationMG<Range> &second_comp)
  ...
  /**
   * Scalar multiplication of the PackagedOperationMG with a @p number.
   */
  PackagedOperationMG<Range> &
  ...
  /**
   * Store the result of the PackagedOperationMG in a vector v of the @p Range
   * space.
   */
  std::function<void(Range &v)> apply;

  /**
   * Add the result of the PackagedOperationMG to a vector v of the @p Range
   * space.
   */
...
/**
 * @relatesalso PackagedOperationMG
 *
 * Addition of two PackagedOperationMG objects @p first_comp and @p second_comp
...
template <typename Range>
PackagedOperationMG<Range>
operator+(const PackagedOperationMG<Range> &first_comp,
          const PackagedOperationMG<Range> &second_comp)
{
  PackagedOperationMG<Range> return_comp;

  return_comp.reinit_vector = first_comp.reinit_vector;

  // ensure to have valid PackagedOperationMG objects by catching first_comp and
 ...
 * @relatesalso PackagedOperationMG
 *
 * Subtraction of two PackagedOperationMG objects @p first_comp and @p
 ...
 * @relatesalso PackagedOperationMG
 *
 * Scalar multiplication of a PackagedOperationMG objects @p comp with a scalar
 * @p number given by a scaling PackagedOperationMG result with @p number.
 *
 * @ingroup LAOperators
 */
template <typename Range>
PackagedOperationMG<Range>
operator*(const PackagedOperationMG<Range> &comp,
...
PackagedOperationMG<Range>
operator*(typename Range::value_type        number,
          const PackagedOperationMG<Range> &comp)
{
  return comp * number;
}
...
template <typename Range>
PackagedOperationMG<Range>
operator+(const PackagedOperationMG<Range> &comp, const Range &offset)
{
  return comp + PackagedOperationMG<Range>(offset);
}
...
template <typename Range>
PackagedOperationMG<Range>
operator+(const Range &offset, const PackagedOperationMG<Range> &comp)
{
  return PackagedOperationMG<Range>(offset) + comp;
}
 ...
 * space). The result is a PackagedOperationMG object that applies this
 * computation.
 *
 * @ingroup LAOperators
 */
...
/**
 * @name Creation of a PackagedOperationMG object
 */
//@{
 ...
 * @relatesalso PackagedOperationMG
 *
 * Create a PackagedOperationMG object that stores the addition of two vectors.
 *
 * The PackagedOperationMG object that is created stores a reference to @p u and
 * @p v. Thus, the vectors must remain valid references for the whole lifetime
 * of the PackagedOperationMG object. All changes made on @p u or @p v after the
 * creation of the PackagedOperationMG object are reflected by the operator
 * object.
 *
 * @ingroup LAOperators
 */
...
operator+(const Range &u, const Range &v)
{
  ...
  // ensure to have valid PackagedOperationMG objects by catching op by value
 ...
 * @relatesalso PackagedOperationMG
 *
 * Create a PackagedOperationMG object that stores the subtraction of two
 * vectors.
 *
 * The PackagedOperationMG object that is created stores a reference to @p u and
 * @p v. Thus, the vectors must remain valid references for the whole lifetime
 ...
 * @relatesalso PackagedOperationMG
 *
 * Create a PackagedOperationMG object that stores the scaling of a vector with
 * a @p number.
 *
 * The PackagedOperationMG object that is created stores a reference to @p u.
 * Thus, the vectors must remain valid references for the whole lifetime of
 * the PackagedOperationMG object. All changes made on @p u or @p v after the
 * creation of the PackagedOperationMG object are reflected by the operator
 * object.
 *
 * @ingroup LAOperators
...
{
  return PackagedOperationMG<Range>(u) * number;
}
...
{
  return number * PackagedOperationMG<Range>(u);
}
 ...
 * @relatesalso PackagedOperationMG
 *
 * Create a PackagedOperationMG object from a LinearOperatorMG and a reference
 * to
 * a vector @p u of the Domain space. The object stores the PackagedOperationMG
...
template <typename Range, typename Domain, typename Payload>
PackagedOperationMG<Range>
operator*(const LinearOperatorMG<Range, Domain, Payload> &op, const Domain &u)
{
  PackagedOperationMG<Range> return_comp;
 ...
 * Create a PackagedOperationMG object from a LinearOperatorMG and a reference
 * to
 * a vector @p u of the Range space. The object stores the PackagedOperationMG
 ...
 */
template <typename Range, typename Domain, typename Payload>
PackagedOperationMG<Domain>
operator*(const Range &u, const LinearOperatorMG<Range, Domain, Payload> &op)
{
  PackagedOperationMG<Range> return_comp;
 ...
 * @relatesalso PackagedOperationMG
 *
 * Composition of a PackagedOperationMG object with a LinearOperatorMG. The
...
operator*(const LinearOperatorMG<Range, Domain, Payload> &op,
          const PackagedOperationMG<Domain>              &comp)
{
  PackagedOperationMG<Range> return_comp;
