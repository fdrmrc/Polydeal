 ...
 * linear operator are also bound to the LinearOperatorMG class on the type
 * level. Because of this, `LinearOperatorMG<Range, Domain>` has two
 ...
 * LinearOperatorMG <code>op</code> that stores the knowledge of this operation,
 * one can write:
 *
 * @code
 ...
 * in conjunction with the LinearOperatorMG class, it is necessary to extend the
 * functionality of the LinearOperatorMG class by means of an additional
 * Payload.
 *
 * For example: LinearOperatorMG instances representing matrix inverses usually
 * require calling some linear solver. These solvers may not have interfaces
 * to the LinearOperatorMG (which, for example, may represent a composite
 * operation). The
 * TrilinosWrappers::internal::LinearOperatorMGImplementation::TrilinosPayload
 * therefore provides an interface extension to the LinearOperatorMG so that it
 ...
 * LinearOperatorMG class are when composite operations are constructed (via
 ...
 *
 * @note Many use cases of LinearOperatorMG lead to intermediate expressions
...
public:
  /**
   * Create an empty LinearOperatorMG object.
  ...
  /**
   * Templated copy constructor that creates a LinearOperatorMG object from an
   * object @p op for which the conversion function
   * <code>linear_operator_mg</code> is defined.
   */
   ...
   * conversion function <code>linear_operator_mg</code> is defined.
   */
  template <typename Op,
            typename = std::enable_if_t<
              !std::is_base_of_v<LinearOperatorMG<Range, Domain, Payload>, Op>>>
  ...
  /**
   * Application of the LinearOperatorMG object to a vector u of the @p Domain
  ...
  /**
   * Application of the transpose LinearOperatorMG object to a vector u of the
   * @p Range space giving a vector v of the @p Domain space.
   */
  std::function<void(Domain &v, const Range &u)> Tvmult;

  /**
   * Application of the transpose LinearOperatorMG object to a vector @p u of
  ...
  /**
   * Addition with a LinearOperatorMG @p second_op with the same @p Domain and
   * @p Range.
   */
  LinearOperatorMG<Range, Domain, Payload> &
  ...
  /**
   * Subtraction with a LinearOperatorMG @p second_op with the same @p Domain
   * and @p Range.
   */
  LinearOperatorMG<Range, Domain, Payload> &
  operator-=(const LinearOperatorMG<Range, Domain, Payload> &second_op)
  ...
  /**
   * Composition of the LinearOperatorMG with an endomorphism @p second_op of
   * the @p Domain space.
   */
  LinearOperatorMG<Range, Domain, Payload> &
  ...
  /**
   * Scalar multiplication of the LinearOperatorMG with @p number from the
   * right.
   */
  LinearOperatorMG<Range, Domain, Payload>
...
/**
 * @name Composition and manipulation of a LinearOperatorMG
 */
/** @{ */
 ...
 * @relatesalso LinearOperatorMG
 *
 * Return an object representing the inverse of the LinearOperatorMG @p op.
 *
 ...
 * LinearOperatorMG object.
 *
 * The LinearOperatorMG object that is created stores a reference to @p solver
 * and @p preconditioner. Thus, both objects must remain a valid reference for
 * the whole lifetime of the LinearOperatorMG object. Internal data structures
 ...
 * @relatesalso LinearOperatorMG
 *
 * Variant of above function that takes a LinearOperatorMG @p preconditioner
...
/**
 * @name Creation of a LinearOperatorMG
 */
/** @{ */
 ...
 * @relatesalso LinearOperatorMG
 *
 * Return a LinearOperatorMG that is the identity of the vector space @p Range.
 *
 ...
 * <code>reinit_domain_vector</code> objects of the LinearOperatorMG object.
 *
 * @ingroup LAOperators
 */
template <
 ...
 * Return a LinearOperatorMG that is the identity of the vector space @p Range.
 *
 * The function takes a LinearOperatorMG @p op and uses its range initializer
 ...
 * @relatesalso LinearOperatorMG
 *
 * Return a nulled variant of the LinearOperatorMG @p op, i.e. with optimized
 * LinearOperatorMG::vmult, LinearOperatorMG::vmult_add, etc. functions and with
 * LinearOperatorMG::is_null_operator set to true.
 *
 * @ingroup LAOperators
 */
template <typename Range, typename Domain, typename Payload>
 ...
 * @relatesalso LinearOperatorMG
 *
 * Return a LinearOperatorMG that acts as a mean value filter. The vmult()
...
namespace internal
{
  namespace LinearOperatorMGImplementation
  {
    ...
    /**
     * A dummy class for LinearOperatorMGs that do not require any extensions to
     ...
     * necessary to initialize a LinearOperatorMG with their associated Payload.
     *
     *
     * @ingroup LAOperators
     */
     ...
     * multiplication of two LinearOperatorMGs
     */
    inline EmptyPayload
    operator*(const EmptyPayload &, const EmptyPayload &)
    {
 ...
 * compatible Vector type into a LinearOperatorMG. The LinearOperatorMG object
 * that is created stores a reference to the matrix object. Thus, @p matrix
 * must remain a valid reference for the whole lifetime of the LinearOperatorMG
 * object.
 *
 * All changes made on @p matrix after the creation of the LinearOperatorMG
 * object are reflected by the operator object. For example, it is a valid
 * procedure to first create a LinearOperatorMG and resize, reassemble the
 * matrix later.
 *
 ...
 * @relatesalso LinearOperatorMG
 *
 * Variant of above function that takes a LinearOperatorMG @p
